...
class IntelligentQADemo:
    ...
    def __init__(self):
        self.rag_tool = RAGTool(
            knowledge_base_path="./qa_demo_kb", rag_namespace="intelligent_qa_demo"
        )
...
""",
    ...
    def _calculate_quality_score(
        self, answer: str, expected_aspects: list, response_time: float
    ) -> float:
        """计算答案质量评分"""
        score = 0.0
...
[引用]标注信息来源和依据""",
        ...
        for strategy in prompt_strategies:
            ...
            start_time = time.time()
            answer = self.rag_tool.run(
                {"action": "ask", "question": test_question, "limit": 3}
            )
    ...
    def demonstrate_citation_system(self):
        ...
        for question in citation_test_questions:
            ...
            # 启用引用的问答
            answer_with_citations = self.rag_tool.run(
                {
                    "action": "ask",
                    "question": question,
                    "limit": 3,
            ...
            # 禁用引用的问答对比
            answer_without_citations = self.rag_tool.run(
                {
                    "action": "ask",
                    "question": question,
                    "limit": 3,
