...
3. 使用 pipdeptree 分析依赖树""",
    ...
    # 2. 读取笔记
    print("2. 读取笔记...")
    note_detail = notes.run({"action": "read", "note_id": note_id_1})
    print(note_detail + "\n")
...
先解决依赖冲突,再继续重构业务逻辑层""",
    ...
    # 4. 搜索笔记
    print("4. 搜索笔记...")
    search_results = notes.run({"action": "search", "query": "依赖", "limit": 5})
    print(search_results + "\n")

    # 5. 列出笔记
    print("5. 列出所有 blocker 类型的笔记...")
    blockers = notes.run({"action": "list", "note_type": "blocker", "limit": 10})
    print(blockers + "\n")
