...
async def get_npc_memories(npc_name: str, limit: int = 10):
    ...
    try:
        ...
        return {"npc_name": npc_name, "memories": memories, "total": len(memories)}
...
async def get_npc_affinity(npc_name: str, player_id: str = "player"):
    ...
    try:
        ...
        return {"npc_name": npc_name, "player_id": player_id, **affinity_info}

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"获取好感度失败: {str(e)}")
...
async def get_all_affinities(player_id: str = "player"):
    ...
    try:
        ...
        return {"player_id": player_id, "affinities": affinities}
...
async def set_npc_affinity(npc_name: str, affinity: float, player_id: str = "player"):
    ...
    if affinity < 0 or affinity > 100:
        raise HTTPException(status_code=400, detail="好感度必须在0-100之间")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"设置好感度失败: {str(e)}")
