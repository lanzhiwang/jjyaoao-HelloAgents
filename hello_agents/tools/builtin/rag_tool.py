...
class RAGTool(Tool):
    ...
    def _preprocess_parameters(self, action: str, **kwargs) -> Dict[str, Any]:
        ...
        elif action in ["search"] and not (
            kwargs.get("query") or kwargs.get("question")
        ):
        ...
        if citations:
            ...
            for citation in citations:
                score_emoji = (
                    ...
                    else "🟡" if citation["score"] > 0.6 else "🔵"
                )
                result.append(
    ...
    def _clear_knowledge_base(
        self, confirm: bool = False, namespace: Optional[str] = None, **kwargs
    ) -> str:
        """清空知识库"""
        try:
            if not confirm:
