...
说明(中文):
- 提供统一的文本嵌入接口与多实现: 本地 Transformer、DashScope(通义千问)、TF-IDF 兜底.
...
- EMBED_MODEL_NAME: 模型名称(dashscope 默认 text-embedding-v3; local默认 sentence-transformers/all-MiniLM-L6-v2)
...
class LocalTransformerEmbedding(EmbeddingModel):
    """本地 Transformer 嵌入(优先 sentence-transformers, 缺失回退 transformers + torch)"""
...
class TFIDFEmbedding(EmbeddingModel):
    ...
    def encode(self, texts: Union[str, List[str]]):
        if not self._is_fitted:
            raise ValueError("TF-IDF 模型未训练, 请先调用 fit() 方法")
        if isinstance(texts, str):
            texts = [texts]
            single = True
        else:
...
class DashScopeEmbedding(EmbeddingModel):
    """阿里云 DashScope(通义千问)Embedding / OpenAI 兼容 REST 模式
