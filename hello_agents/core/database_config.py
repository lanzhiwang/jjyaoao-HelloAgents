...
class QdrantConfig(BaseModel):
    ...
    collection_name: str = Field(
        default="hello_agents_vectors", description="向量集合名称"
    )
    vector_size: int = Field(default=384, description="向量维度")
    distance: str = Field(
        default="cosine", description="距离度量方式 (cosine, dot, euclidean)"
    )
