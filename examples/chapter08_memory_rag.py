...
def demo_simple_agent_with_memory():
    ...
    memory_tool = MemoryTool(
        user_id="demo_user_001", memory_types=["working", "episodic", "semantic"]
    )
...
def demo_simple_agent_with_rag():
    ...
    # 创建RAG工具 - 使用本地嵌入(推荐)
    rag_tool = RAGTool(knowledge_base_path="./demo_knowledge_base")
...
3. 基于搜索结果给出专业回答""",
    ...
    for content, doc_id in knowledge_items:
        result = rag_tool.run(
            {"action": "add_text", "text": content, "document_id": doc_id}
        )
        print(f"  ✅ 已添加: {doc_id}")
...
def demo_combined_memory_and_rag():
    ...
    # 创建记忆工具
    memory_tool = MemoryTool(
        user_id="combo_user", memory_types=["working", "episodic", "semantic"]
    )
...
def demo_four_memory_types():
    ...
    for query, desc in search_queries:
        print(f"\n  {desc} ('{query}'):")
        result = memory_tool.run(
            {"action": "search", "query": query, "limit": 3, "min_importance": 0.5}
        )
        print(f"    {result}")
...
def demo_advanced_features():
    ...
    for i, text in enumerate(knowledge_batch):
        result = rag_tool.run(
            {"action": "add_text", "text": text, "document_id": f"ai_knowledge_{i}"}
        )
        print(f"    添加文档 {i+1}: ✅")
    ...
    for query in search_queries:
        result = rag_tool.run(
            {"action": "search", "query": query, "limit": 2, "min_score": 0.1}
        )
...
def demo_enhanced_pdf_and_local_embedding():
    ...
    if pdf_files:
        ...
        ask_result = rag_tool.run(
            {"action": "ask", "question": test_query, "namespace": "pdf_test"}
        )
        print(f"\n🤖 智能问答:")
        print(ask_result)
...
def demo_real_world_scenario():
    ...
    rag_tool = RAGTool(
        knowledge_base_path="./learning_assistant_kb", embedding_model="local"
    )
